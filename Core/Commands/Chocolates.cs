...
namespace AHH_Bot.Commands
{
    ...
    public class Chocolates : ModuleBase<SocketCommandContext>
    {
        ...
        {
            if (user == null)
                await Context.Channel.SendMessageAsync($"{Context.User.Mention} you currently have collected {Data.Users.Chocolates.GetChocolateAmount(Context.User.Id)} chocolates :chocolate_bar:");
            else
                await Context.Channel.SendMessageAsync($"{user.Mention} has collected {Data.Users.Chocolates.GetChocolateAmount(user.Id)} chocolates :chocolate_bar:");
        }
        ...
        public async Task spawnChoco(IUser user = null, int chocoAmount = 0)
        {
            ...
            await Data.Users.Chocolates.AddChocolates(user.Id, chocoAmount);
            await Context.Channel.SendMessageAsync($":white_check_mark: You've successfully added {chocoAmount} chocolates to {user.Mention}'s account.");
        }
        ...
        public async Task giftChoco(IUser user = null, int chocoAmount = 0)
        {
            ...
            if (Data.Users.Chocolates.GetChocolateAmount(Context.User.Id) - chocoAmount < 0)
            {
            ...
            await Data.Users.Chocolates.AddChocolates(Context.User.Id, -chocoAmount);
            await Data.Users.Chocolates.AddChocolates(user.Id, chocoAmount);
        }
        ...
        public async Task GetTop(int top = 3)
        {
            if (top < 1)
            {
                var errormsg = await Context.Channel.SendMessageAsync(":x: Input a valid number.");
                await Task.Delay(Settings.ErrorMessageTime);
                await Context.Message.DeleteAsync();
                await errormsg.DeleteAsync();
                return;
            ...
            if (top > 50)
            {
                var errormsg = await Context.Channel.SendMessageAsync(":x: Number must be under 50.");
                await Task.Delay(Settings.ErrorMessageTime);
                await Context.Message.DeleteAsync();
                await errormsg.DeleteAsync();
                return;
            ...
            string temp = "";
            foreach (var user in Data.Users.Chocolates.GetTopChoco(top))
            {
                var guildUser = Context.Client.GetUser(user);
                if (!guildUser.IsBot)
                    temp += $"{guildUser.Username} - {Data.Users.Chocolates.GetChocolateAmount(user)}\n";
            }
        ...
        public class Reset : Chocolates
        {
            ...
            {
                await Data.Users.Chocolates.SetChocolates(Context.User.Id, 0);
            }
            ...
            public async Task UserReset(IUser user = null)
            {
                ...
                await Data.Users.Chocolates.SetChocolates(user.Id, 0);
